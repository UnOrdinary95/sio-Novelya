...
import {
    deleteOneLightNovel,
    findAllLightNovels,
    findAllLightNovelsByGenre,
    findOneLightNovel,
    insertOneLightNovel,
    updateOneLightNovel,
    findLightNovelsByName,
} from '../models/lightNovelModel.js';
...
// READ
export const getLightNovels = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
        const lightNovels = await findAllLightNovels();
        const response = lightNovels.map((ln) => ({
            ...ln,
            cover: ln.cover ? buildCoverUrl(ln.cover) : null,
        }));
        res.status(200).json(response);
    } catch (err) {
        logger.error('getLightNovels', err);
        res.status(500).json({
            error: 'Erreur lors de la récupération des light novels',
        });
    }
};

export const getLightNovelsByGenre = async (
    req: Request,
    res: Response
): Promise<void> => {
    ...
    try {
        ...
        res.status(500).json({
            error: 'Erreur lors de la récupération des light novels par genre',
        });
    }
};

export const getLightNovelById = async (
    req: Request,
    res: Response
): Promise<void> => {
    const lightNovelId = req.params.id as string;
    try {
        const lightNovel = await findOneLightNovel(lightNovelId);
        const response = {
            ...lightNovel,
            cover: lightNovel.cover ? buildCoverUrl(lightNovel.cover) : null,
        };
        res.status(200).json(response);
    } catch (err) {
        logger.error('getLightNovelById', err);
        res.status(500).json({
            error: 'Erreur lors de la récupération du light novel',
        });
    }
};

export const searchLightNovelByName = async (
    req: Request,
    res: Response
): Promise<void> => {
    ...
    try {
        ...
        res.status(500).json({
            error: 'Erreur lors de la recherche des light novels',
        });
    }
};

// UPDATE
export const updateLightNovelById = async (
    req: Request,
    res: Response
): Promise<void> => {
    const lightNovelId = req.params.id as string;
    ...
    try {
        const updatedLightNovel = await updateOneLightNovel(
            lightNovelId,
            lightNovelData
        );
        ...
        res.status(500).json({
            error: 'Erreur lors de la mise à jour du light novel',
        });
    }
};

export const uploadCoverById = async (
    req: Request,
    res: Response
): Promise<void> => {
    if (!req.file) {
...
// DELETE
export const deleteLightNovelById = async (
    req: Request,
    res: Response
): Promise<void> => {
    const lightNovelId = req.params.id as string;
    ...
    try {
        ...
        res.status(500).json({
            error: 'Erreur lors de la suppression du light novel',
        });
    }
};
